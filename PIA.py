...
import pandas as pd
# Importamos los modulos sys y sqlite3 para la creacion de la base de datos
import sys
import sqlite3
...
# Se agregan valores a la tupla nominada junto a otras listas
            ...
            print()
            print(f"El cliente {nombre_cliente} debe pagar un monto total con IVA incluido de: ${montosCobrados}")
            print()
            break
        
    try:
        with sqlite3.connect("Consultas_PIA.db") as conn:
            mi_cursor = conn.cursor()
            mi_cursor.execute("CREATE TABLE IF NOT EXISTS evidencia (servicio PRIMARY KEY, equipos TEXT NOT NULL)")
            detalles = [descripcion_del_equipo, servicio]
            mi_cursor.execute("INSERT INTO evidencia VALUES(?,?)", detalles)
